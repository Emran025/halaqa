...
يُبنى التطبيق كعميل **WPF Application بلغة C# لسطح Windows** يعمل **على الإنترنت افتراضياً** ويتصل بخدمة ويب Laravel عبر عقد OpenAPI، وبنمط Clean Architecture متكرر داخل كل ميزة. تتبع كل ميزة ثلاث طبقات واضحة: `Presentation` لإدارة العرض والحالة، و`Domain` للسياسات والكائنات وحالات الاستخدام وعقود المستودعات، و`Data` لمصادر البيانات وتحويل JSON وتنفيذ المستودعات. يوجد مجلد `Config` عابر للميزات للتهيئة فقط، ومجلد `Shared` لمكونات العرض والسمات والتفاعل المشتركة فقط.
...
|---|---|---|---|
| المصحف | نسخة قراءة فقط من `QuranV3.sqlite` المأخوذة من المرجع المعتمد، وتحتوي بيانات السور والآيات ووحدات المتابعة | لا يحتاج لطلب شبكة للعرض المحلي؛ API هو المرجع لحالة الجلسة الرسمية فقط | يسمح بالقراءة دون اتصال؛ لا يعامل SQLite المحلي كمصدر حقيقة رسمي لحالة الجلسة. |
| الأخطاء | جدول SQLite محلي `mistake_outbox` للعمليات المعلقة وحالة المزامنة و`client_operation_id` | يزامن العملية وفق ترتيبها ويحترم 409 و422 | لا تضيع العلامة؛ تعرض Pending/Conflict/Failed بوضوح، ولا تحذف العملية إلا بعد تأكيد الخادم. |
...
يكون لكل عملية خطأ `client_operation_id` ثابت يولد عند الإنشاء. عند فشل الشبكة، تسجل في جدول SQLite محلي باسم `mistake_outbox` مع نوع العملية والحمولة ووقت الإنشاء وحالة المزامنة. يعيد `MistakesSyncService` المحاولات عند عودة الاتصال أو عند طلب المستخدم، مع تسلسل operations للمورد نفسه. لا يعاد تطبيق عملية إلا إذا كانت قابلة للإعادة وفق العقد.
