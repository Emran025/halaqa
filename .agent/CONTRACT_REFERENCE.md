...
| Sessions | `/sessions`، `/sessions/*`، `/sessions/{sessionId}/mushaf-state`، `/sessions/{sessionId}/realtime`، `/realtime/channels/authorize` | يستهلك العميل قائمة الجلسات الرسمية المرقمة والمفلترة، إضافة إلى realtime config وchannel authorization و**حفظ** mushaf state. لا يثبت ذلك WebRTC أو WebSocket أو مهام أو اتصال وسائط؛ وتبقى شاشة الجلسة الحية غير موصولة بالقائمة. |
