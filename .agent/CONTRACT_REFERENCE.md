...
| Sessions | `/sessions`، `GET/POST /sessions/{sessionId}/tasks`، `/sessions/*`، `/sessions/{sessionId}/mushaf-state`، `/sessions/{sessionId}/realtime`، `/realtime/channels/authorize` | يستهلك العميل قائمة الجلسات الرسمية المرقمة والمفلترة ومهام الجلسة للقراءة، ويتيح للمعلم إنشاء مهمة بحدها الإلزامي (`task_type` و`client_operation_id`) فقط، إضافة إلى realtime config وchannel authorization و**حفظ** mushaf state. لا يثبت ذلك WebRTC أو WebSocket أو تعديل مهمة أو اتصال وسائط؛ وتبقى شاشة الجلسة الحية غير موصولة بالقائمة. |
| Mistakes | `/sessions/{sessionId}/tasks/{taskId}/mistakes` | توجد واجهة تسجيل للطرفين المصرح لهما تصل من مهمة الجلسة المختارة؛ تتحقق من الموضع ثم تستخدم outbox القائم وتحاول المزامنة، مع إظهار حالة synced/pending/conflict/failed الفعلية. لا توجد ملاحظات أو تقييمات أو تحقق API حي موثق بعد. |
