...
| Sessions | `/sessions`، `/sessions/{sessionId}/tasks`، `/sessions/*`، `/sessions/{sessionId}/mushaf-state`، `/sessions/{sessionId}/realtime`، `/realtime/channels/authorize` | يستهلك العميل قائمة الجلسات الرسمية المرقمة والمفلترة ومهام الجلسة للقراءة، إضافة إلى realtime config وchannel authorization و**حفظ** mushaf state. لا يثبت ذلك WebRTC أو WebSocket أو إنشاء/تعديل مهمة أو اتصال وسائط؛ وتبقى شاشة الجلسة الحية غير موصولة بالقائمة. |
